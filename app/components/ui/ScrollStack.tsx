...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    // Reset styles to clean state before measuring offsets!
    cards.forEach((card) => {
    ...
    // Measure and cache offsets
    cardOffsetsRef.current = cards.map(card => getAbsoluteOffset(card));

    const endElement = useWindowScroll
      ? document.querySelector('.scroll-stack-end')
      : scroller.querySelector('.scroll-stack-end');
    endElementOffsetRef.current = endElement ? getAbsoluteOffset(endElement as HTMLElement) : 0;

    // Re-apply required style properties
    ...
    const handleResize = () => {
      // Temporarily clear transforms to measure true static offsets
      const currentTransforms = cards.map(c => c.style.transform);
      const currentFilters = cards.map(c => c.style.filter);
      ...
      const endEl = useWindowScroll
        ? document.querySelector('.scroll-stack-end')
        : scroller.querySelector('.scroll-stack-end');
      endElementOffsetRef.current = endEl ? getAbsoluteOffset(endEl as HTMLElement) : 0;

      // Restore transforms
      cards.forEach((c, idx) => {
        c.style.transform = currentTransforms[idx];
        c.style.filter = currentFilters[idx];
      });
